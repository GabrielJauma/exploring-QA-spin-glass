...
N_p = 1e6;                    % Nº pasos Monte Carlo totales, los primeros
...
N_durante_eq=2e4;             % Numero de pasos utilizados para promediar las variables una vez el sistema esta termalizado.
N_mc=f*c*2;                   % Numero de pasos que tienen que pasar sin que se acepte ningun cambio M-MC para que se considere termalizado. 
...
for i = 0:N_p
    ...
    if i>k_mc+N_mc && eq==0
        ...
        N_p2(T)=N_eq(T)+N_durante_eq;
        eq=1;
        fprintf('Equilibrio para T=%.2f alcanzado en %d pasos por exceso de pasos sin cambio \n',temps(T),i)
    end
