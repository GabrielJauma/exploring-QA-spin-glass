...
N_t = round(100*n);       % A cada N_t pasos se mide E. Las medidas se usan para decidir sobre la termalización.
...
N_indep =10*n; %round(sqrt(n)); % A cada N_indep se considera que el sistema ha evolicionado lo suficiente como para tomar otra muestra del mismo y utilizarla
...
Jm=Jmatrix(f,c,modelo,seed*conf,RNG,J,J0);
...
while i<N_p2(T)
    ...
    if parallel_tempering && T>1 && send_tempering
            labSend(s,T-1,0);
    end
...
hold on
for ii=1:size(dim,1)
Bf=fit(temps',Bd(ii,:)','gauss2');
plot(temps,Bd(ii,:),'.','markersize',10)
plot(Bf,'k')
ylabel('$B$','interpreter','latex')  
end
