...
if opt_inicial ==2                               
opt_p_dw=-5;                   % Si opt_p_dw pertenece a [0,1] entonces p_dw = opt_p_dw. Sinó se define un 
end                            % p_dw(temperatura) de tal modo que la matriz de espines resulte en  
...
N_mean=2e3;                    % Calcula la media de E y M a cada N_mean pasos y decide si esta termalizado o no.
...
error_E=error_E*(1-exp(-temps)); % Error en la E para que se acepte como estado de equilibrio.
error_M=error_M*(1-exp(-temps)); % Error en la M ...
