...
%% Parámetros físicos
f = 16;                         % Nº filas matriz de espines, ha de ser par.
...
L =[6 10];                % Barrido de simulaciones para varias dimensiones [Lf x Lc]. 
                          % en SK 6 y 8 cruzan dpm, 8 y 10 van paralelas 20 corta
%L =[3 5]% 5 6]; %modelo 3
...
while i<N_p2 % While exterior que permite hacer parallel tempering
    ...
    for i = last_i+1:last_i+N_pt   
        ...
        if dE<=0 || rand<exp(-dE/(kb*temps(T)))
            ...
            % (f*(c_p-1) + f_p) es el indice correspondiente a s(f_p,c_p)
        ...
        for t=1:(N_durante_eq/N_indep)
            for a = 1:N_replicas
             sa = S(:,t,a,T);
                for b = 1:N_replicas
                    sb    = S(:,t,b,T);
                    qab(a,b,t) = mean(sa.*sb);
                    qab2(a,b,t) = mean(sa.*sb)^2;
                    qab4(a,b,t) = qab2(a,b,t)^2;
                end
            end
        end
        %Thermal average
        qab_t_av   = mean(qab,3);
        qab2_t_av   = mean(qab2,3);
        qab4_t_av   = mean(qab4,3);
        q4q2_t_av = qab4_t_av./(qab2_t_av.^2);
        %Replica average
        qab_r_av(T) = mean(qab_t_av, [1 2])*N_replicas^2/( N_replicas^2 -N_replicas)-N_replicas/( N_replicas^2 -N_replicas);
        qab2_r_av(T) = mean(qab2_t_av, [1 2])*N_replicas^2/( N_replicas^2 -N_replicas)-N_replicas/( N_replicas^2 -N_replicas);
        qab4_r_av(T) = mean(qab4_t_av, [1 2])*N_replicas^2/( N_replicas^2 -N_replicas)-N_replicas/( N_replicas^2 -N_replicas);
        q4q2_r_av(T) = mean(q4q2_t_av, [1 2])*N_replicas^2/( N_replicas^2 -N_replicas)-N_replicas/( N_replicas^2 -N_replicas);
        
        B2_r_av(T) =  0.5*(3 - q4q2_r_av(T));
        B_r_av(T) =  0.5*(3 - qab4_r_av(T)/qab2_r_av(T)^2);
    end   
end

if use_conf
    Ec     = zeros(N_conf,N_temps);
    Mc     = zeros(N_conf,N_temps);
    Cvc    = zeros(N_conf,N_temps);
    Xc     = zeros(N_conf,N_temps);
    qc     = zeros(N_conf,N_temps);
    qab_c  = zeros(N_conf,N_temps);
    qab2_c = zeros(N_conf,N_temps);
    qab4_c = zeros(N_conf,N_temps);
    ...
    if use_replicas
        qab_c(conf,:)= qab_r_av;
        qab2_c(conf,:)= qab2_r_av;
        qab4_c(conf,:)= qab4_r_av;
    end
end
...
if use_conf
    ...
    q = mean(qc); 
    if use_replicas
        qab_c_av = mean(qab_c,1);
        qab2_c_av=mean(qab2_c,1);
        qab4_c_av=mean(qab4_c,1);
        q4q2_c_av=mean(qab4_c_av./(qab2_c_av.^2),1);

        B_c_av = 0.5*(3 - qab4_c_av./(qab2_c_av.^2) );
        B2_c_av = 0.5*(3 - q4q2_c_av);
    end
end
...
if use_replicas && ~use_conf
    figure
    plot(temps,qab_r_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$q_{\alpha\beta}$','interpreter','latex')
    
    figure
    plot(temps,B_r_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$B$','interpreter','latex')
    
    figure
    plot(temps,B2_r_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$B*$','interpreter','latex')
    
elseif use_replicas &&  use_conf 
    figure
    plot(temps,qab_c_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$q_{\alpha\beta}$','interpreter','latex')
    
    figure
    plot(temps,B_c_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$B$','interpreter','latex')

    figure
    plot(temps,B2_c_av,'-ok','markersize',3,'linewidth',2,'Color',[0.5,0.5,0.5],'MarkerEdgeColor','k','MarkerFaceColor','k')
    ylabel('$B*$','interpreter','latex')
end
...
hold on
for ii=1:size(dim,1)
semilogy(temps,qd(ii,:),'-o','markersize',2,'color',colors(ii,:),'linewidth',2,'DisplayName',['$n=$' num2str(N(ii))])
end
legend
...
hold on
for ii=1:size(dim,1)
semilogy(temps,qabd(ii,:),'-o','markersize',2,'color',colors(ii,:),'linewidth',2,'DisplayName',['$n=$' num2str(N(ii))])
end
legend
ylabel('$\overline{q_{\alpha\beta}}$','interpreter','latex')
xlabel('$T$','interpreter','latex')  
set(gca,'fontsize',14)
print(['Figuras\qab_escalado_' num2str(modelo) '.png'],'-dpng','-r500')
...
hold on
for ii=1:size(dim,1)
semilogy(temps,B2d(ii,:),'-o','markersize',2,'color',colors(ii,:),'linewidth',2,'DisplayName',['$n=$' num2str(N(ii))])
end
legend
ylabel('$B*$','interpreter','latex')
xlabel('$T$','interpreter','latex')  
