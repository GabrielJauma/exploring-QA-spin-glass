...
modelo = 2;                     % 1 = Ising 2D con interacci�n a primeros vecinos, 
...
%% Par�metros f�sicos
f = 6;                         % N� filas matriz de espines, ha de ser par.
...
T_max=3;                     % L�mite superior del barrido de temperaturas de la simulaci�n.
...
%L =[3 4 6];% 6];                % Barrido de simulaciones para varias dimensiones [Lf x Lc]. 
...
%% Finding best v exponent and Tc for binder cumulant colapse 1
T_cutoff_up=1;
T_cutoff_dw=3;
last_error=10;
K_max=200;
...
for K=1:K_max
        ...
        t_exp=[];
        for i=1:length(N)
        g_exp = [ g_exp;  Bd(i,3:end-10)'                     ];
        t_exp = [ t_exp; (temps(3:end-10)'-T_c_test)*N(i)^x_test ];
        end
...
%% Finding best v exponent and Tc for binder cumulant colapse 2
T_cutoff_up=5;
T_cutoff_dw=0;
last_error=10;
K_max=100;
