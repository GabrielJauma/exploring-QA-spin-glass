...
@njit(void(int32[::1], float64[::1], float64[::1], float64[::1]),
...
@njit(void(float64[::1], float64[::1], float64[::1], int8[:,::1],
           float64[::1], int32[::1]),
      boundscheck=False, fastmath=True, nogil=True)
def mc_step(p, βE, E, s, dE, change):
    for i, (βi, dEi, pi, ci) in enumerate(zip(βE, dE, p, change)):
        if dEi < 0 or pi <= math.exp(-βi*dEi):
            s[i, ci] *= -1
            E[i] += dEi
...
@njit(Tuple((int8[:,::1],float64[::1]))
           (float64[::1], sparse_type, int8[:,::1], float64[::1], intc,
            int32[:,::1], float64[:,::1]),
      locals={'flip_sites': int32[::1], },
      boundscheck=False, fastmath=True, nogil=True)
def mc_loop(β, Jsparse, s, E, steps, random_changes, random_chances):
    copies, n = s.shape
    swap = np.arange(copies, dtype=np.int32)
    k = 0
    dE = np.empty(E.size, dtype=np.double)
    for step in range(steps):
        flip_sites = random_changes[step,:]
        delta_cost(dE, s, Jsparse, flip_sites)
        mc_step(random_chances[k,:], β, E, s, dE, flip_sites)
        k += 1
        if step % 10 == 0:
            tempering_step(swap, random_chances[k,:], β, E)
            k += 1
            E = E[swap]
            s = s[swap, :]
    return s, E
...
def mc_evolution(β, J, steps=None, start=None, eq_points=1000, rng=None):
    ...
    while not thermal_equilibrium:
        random_sites = rng.integers(0, size, (steps, copies), dtype=np.int32)
        random_chances = rng.random((steps + steps//10, copies))
        s, E = mc_loop(β, Jsparse, s, E, steps,
                       random_sites, random_chances)
